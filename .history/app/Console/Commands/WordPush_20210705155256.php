...
class WordPush extends Command
{
     ...
     */
    //コマンドの名前
    protected $signature = 'WordPush {player_id}';//引数を指定する時は {user} のように {} で囲む
    ...
    {
        $player_id = $this->argument('player_id');//引数で落ちてくる user を取得するには
        $player_ = Push::find($user_id);//Wordの引数を設定して、idを入力したらuserが取得するかどうか調べる
        // logger($user);
