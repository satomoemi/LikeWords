                        ...
                        @foreach($folders as $folder)
                            <tr  class="d-flex justify-content-between align-items-center">
                                ...
                                @if($current_folder != NULL)
                                    ...
                                    class="{{ $current_folder === $folder->id ? 'active' : '' }}">
                                        {{ $folder->title }}    
                                    </a>
                                    @else
                                    <a href="{{ route('home', ['id' => $folder->id]) }}" >
        ...
        <div class="col col-md-7">
            <table class="table table-white">
                <thead class="thead thead-dark">
                    <tr>
                        <th>
                            <a class="btn btn-primary" href="{{ route('create.word', ['id' => $current_folder]) }}">Word作成</a>
                        </th>
                    </tr>
                </thead>
                ...
                <tbody>
                    @foreach($words as $word)
                        <tr class="d-flex justify-content-between align-items-center">
                            <th>
                    @if($current_folder != NULL)
                                <a href="{{ route('home', ['id' => $folder->id]) }}" class="{{ $current_folder === $folder->id ? 'active' : '' }}">
                                    {{ $word->word }}
                                </a>
                                ＠else
                                <a href="{{ route('home', ['id' => $folder->id]) }}">
